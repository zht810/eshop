...
import com.zht.eshop.inventory.request.impl.ProductInventoryCacheRefreshRequest;
import com.zht.eshop.inventory.request.impl.ProductInventoryDBUpdateRequest;
import com.zht.eshop.inventory.responseModel.ResponseModel;
...
public class ProductInventoryController {
    ...
    public ResponseModel updateProductInventory(@RequestBody ProductInventoryModel productInventory) {
        // 为了简单起见，我们就不用log4j那种日志框架去打印日志了
        // 其实log4j也很简单，实际企业中都是用log4j去打印日志的，自己百度一下
        System.out.println("===========ProductInventoryController日志===========: 接收到更新商品库存的请求，商品id=" + productInventory.getProductId() + ", 商品库存数量=" + productInventory.getInventoryCnt());
        ...
        try {
            ...
            response = new ResponseModel(ResponseModel.SUCCESS);
        } catch (Exception e) {
            e.printStackTrace();
            response = new ResponseModel(ResponseModel.FAILURE);
        }
        return response;
    }
...
**/
    ...
    public ProductInventoryModel getProductInventory(Integer productId) {
        System.out.println("===========ProductInventoryController日志===========: 接收到一个商品库存的读请求，商品id=" + productId);
        ProductInventoryModel productInventory = null;

        try {
            RequstInterface request =
...
//				}
                ...
                if (productInventory != null) {
                    System.out.println("===========ProductInventoryController日志===========: 在200ms内读取到了redis中的库存缓存，商品id=" + productInventory.getProductId() + ", 商品库存数量=" + productInventory.getInventoryCnt());
            ...
            // 直接尝试从数据库中读取数据
            productInventory = productInventoryService.selectById(productId);
            if (productInventory != null) {
                // 将缓存刷新一下
        ...
        productInventoryModel.setInventoryCnt(-1L);
